...
enum RuleSeverity: String {
    ...
    var color: Color {
        switch self {
        case .low: return Color(red: 0.35, green: 0.80, blue: 0.50)
        case .medium: return Color(red: 1.0, green: 0.67, blue: 0.20)
        case .high: return Color(red: 1.0, green: 0.42, blue: 0.16)
        }
    }
}

struct RuleFinding: Identifiable {
    let id = UUID()
    let title: String
    let detail: String
...
struct RecurringPatternInsight: Identifiable {
    let id = UUID()
    let merchant: String
    let amount: Double
    let cadenceDays: Int
...
struct CashFlowForecastPoint: Identifiable {
    let id = UUID()
    let date: Date
    let projectedDailyOutflow: Double
    let projectedCumulativeOutflow: Double
}
...
class TransactionData {
    ...
    private var datedTransactions: [(transaction: SpendingTransaction, date: Date)] {
        let now = Date()
        return groups.flatMap { group in
            guard let date = Self.resolvedDate(forGroupTitle: group.title, now: now) else {
                return [(transaction: SpendingTransaction, date: Date)]()
            }
            return group.transactions.map { tx in
                (transaction: tx, date: date)
            }
        }
    }

    func recognizedMerchantName(from raw: String, fallback: String) -> String {
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        let source = trimmed.isEmpty ? fallback : trimmed
        let normalized = Self.normalizedMerchantToken(source)
        if normalized.isEmpty { return fallback }

        if let domain = MerchantMatcher.shared.domain(for: normalized),
           let mapped = Self.domainDisplayNames[domain] {
            return mapped
        }
    ...
    var ruleFindings: [RuleFinding] {
        ...
        guard !allTransactions.isEmpty else { return findings }

        let impulseTotal = allTransactions.filter { $0.isImpulse }.reduce(0) { $0 + $1.amountValue }
        let impulseRatio = totalSpent > 0 ? (impulseTotal / totalSpent) : 0
        if impulseRatio >= 0.40 {
            findings.append(
                RuleFinding(
                    title: "Impulse Spend Is High",
                    detail: "\(Int(impulseRatio * 100))% of outflow is impulse-tagged. Consider a 24-hour cooldown rule.",
                    severity: .high
                )
            )
            ...
            findings.append(
                RuleFinding(
                    title: "Impulse Spend Rising",
                    detail: "Impulse purchases are \(Int(impulseRatio * 100))% of spend. Keep this under 25% for stability.",
                    severity: .medium
                )
            )
        }

        if let top = topCategories.first, totalSpent > 0 {
            let concentration = top.amount / totalSpent
            if concentration >= 0.50 {
                findings.append(
                    RuleFinding(
                        title: "Category Concentration Risk",
                        detail: "\(top.name) is \(Int(concentration * 100))% of total outflow. Add a weekly cap for this category.",
                        severity: .medium
                    )
                )
            }
        ...
        let rolling = rollingSevenDaySpend()
        if rolling.current > 0, rolling.previous > 0 {
            let growth = (rolling.current - rolling.previous) / rolling.previous
            if growth >= 0.25 {
                findings.append(
                    RuleFinding(
                        title: "Weekly Spend Acceleration",
                        detail: "Last 7 days are up \(Int(growth * 100))% vs prior week. Tighten short-term limits this week.",
                        severity: .high
                    )
                )
            }
        ...
        let subscriptionSpend = allTransactions
            .filter { $0.category == .subscriptions }
            .reduce(0) { $0 + $1.amountValue }
        if totalSpent > 0, (subscriptionSpend / totalSpent) >= 0.20 {
            findings.append(
                RuleFinding(
                    title: "Subscription Load Is Heavy",
                    detail: "Subscriptions are \(Int((subscriptionSpend / totalSpent) * 100))% of outflow. Audit unused plans monthly.",
                    severity: .low
                )
            )
        }
        ...
        if findings.isEmpty {
            findings.append(
                RuleFinding(
                    title: "No Major Budget Violations",
                    detail: "Current behavior is within standard guardrails based on your recent history.",
                    severity: .low
                )
            )
        }
    ...
    var merchantInsights: [MerchantInsight] {
        var groups: [String: MerchantInsight] = [:]

        for tx in allTransactions {
            let normalized = Self.normalizedMerchantToken(tx.title)
            guard !normalized.isEmpty else { continue }

            let domain = MerchantMatcher.shared.domain(for: normalized)
            let key = domain ?? normalized
            let display = domain.flatMap { Self.domainDisplayNames[$0] } ?? Self.titleCase(normalized)

            if let existing = groups[key] {
                groups[key] = MerchantInsight(
                    merchant: existing.merchant,
                    domain: existing.domain,
                    count: existing.count + 1,
                    totalSpent: existing.totalSpent + tx.amountValue
                )
            } else {
                groups[key] = MerchantInsight(
                    merchant: display,
        ...
        return groups.values
            .sorted { lhs, rhs in
                if lhs.totalSpent != rhs.totalSpent { return lhs.totalSpent > rhs.totalSpent }
    ...
    // MARK: - Smart Recurring Detection
    var recurringDetections: [RecurringPatternInsight] {
        let calendar = Calendar.current
        let grouped = Dictionary(grouping: datedTransactions, by: { pair in
            let merchant = Self.normalizedMerchantToken(pair.transaction.title)
            return "\(merchant)|\(String(format: "%.2f", pair.transaction.amountValue))"
        })

        let patterns: [RecurringPatternInsight] = grouped.compactMap { _, rows in
            let deduped = Dictionary(grouping: rows, by: { calendar.startOfDay(for: $0.date) })
            ...
            for index in 1..<deduped.count {
                let days = calendar.dateComponents([.day], from: deduped[index - 1].date, to: deduped[index].date).day ?? 0
                if days > 0 { intervals.append(days) }
            }

            guard intervals.count >= 2 else { return nil }

            let avg = Double(intervals.reduce(0, +)) / Double(intervals.count)
            guard avg >= 6, avg <= 45 else { return nil }

            let variance = intervals.reduce(0.0) { running, value in
                let diff = Double(value) - avg
                return running + (diff * diff)
            } / Double(intervals.count)
            let stdev = sqrt(variance)

            guard stdev <= max(4.0, avg * 0.22) else { return nil }

            let confidence = min(0.99, max(0.55, 1.0 - (stdev / max(avg, 1))))
            let cadence = Int(avg.rounded())
            guard let lastDate = deduped.last?.date else { return nil }
            let next = calendar.date(byAdding: .day, value: cadence, to: lastDate) ?? lastDate

            let tx = deduped.last?.transaction
            return RecurringPatternInsight(
                merchant: tx.map { recognizedMerchantName(from: $0.title, fallback: $0.title) } ?? "Unknown",
                amount: tx?.amountValue ?? 0,
                cadenceDays: cadence,
    ...
    var cashFlowForecastPoints: [CashFlowForecastPoint] {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: Date())
        let lookbackDays = 30
        let horizon = 14

        let recent = datedTransactions.filter { pair in
            let age = calendar.dateComponents([.day], from: pair.date, to: today).day ?? Int.max
            return age >= 0 && age < lookbackDays
        }
        let baselineDaily = recent.reduce(0.0) { $0 + $1.transaction.amountValue } / Double(max(lookbackDays, 1))

        let recurringSeries = forecastRecurringCharges(horizon: horizon, from: today)
        var cumulative = 0.0
        var points: [CashFlowForecastPoint] = []

        for offset in 0..<horizon {
            guard let day = calendar.date(byAdding: .day, value: offset, to: today) else { continue }
            let recurring = recurringSeries[offset, default: 0]
            let daily = baselineDaily + recurring
            cumulative += daily
            points.append(
                CashFlowForecastPoint(
                    date: day,
                    projectedDailyOutflow: daily,
                    projectedCumulativeOutflow: cumulative
                )
            )
        }
    ...
    private func rollingSevenDaySpend() -> (current: Double, previous: Double) {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: Date())
        let rows = datedTransactions

        let current = rows.reduce(0.0) { running, pair in
            let age = calendar.dateComponents([.day], from: pair.date, to: today).day ?? Int.max
            return (age >= 0 && age < 7) ? (running + pair.transaction.amountValue) : running
        }

        let previous = rows.reduce(0.0) { running, pair in
            let age = calendar.dateComponents([.day], from: pair.date, to: today).day ?? Int.max
            return (age >= 7 && age < 14) ? (running + pair.transaction.amountValue) : running
        }

        return (current, previous)
    }

    private func forecastRecurringCharges(horizon: Int, from start: Date) -> [Int: Double] {
        let calendar = Calendar.current
        var dayOffsetsToCharge: [Int: Double] = [:]

        for sub in subscriptions {
            guard let epoch = sub.nextBillingEpoch else { continue }
            var nextDate = calendar.startOfDay(for: Date(timeIntervalSince1970: epoch))
            let amount = max(sub.price, 0)

            while let offset = calendar.dateComponents([.day], from: start, to: nextDate).day, offset < horizon {
                if offset >= 0 {
                    dayOffsetsToCharge[offset, default: 0] += amount
                }

                nextDate = Self.advanceRecurringDate(
                    current: nextDate,
                    frequencyKey: sub.frequencyKey,
                    fallbackDays: sub.frequencyDays
                )
            }
        }

        for pattern in recurringDetections {
            var date = calendar.startOfDay(for: pattern.nextExpectedDate)
            while let offset = calendar.dateComponents([.day], from: start, to: date).day, offset < horizon {
                if offset >= 0 {
                    dayOffsetsToCharge[offset, default: 0] += pattern.amount
                }
                date = calendar.date(byAdding: .day, value: pattern.cadenceDays, to: date) ?? date
            }
        }

        return dayOffsetsToCharge
    }

    private static let domainDisplayNames: [String: String] = [
        "spotify.com": "Spotify",
        "netflix.com": "Netflix",
        "youtube.com": "YouTube",
        "apple.com": "Apple",
    ...
    private static func normalizedMerchantToken(_ raw: String) -> String {
        let lowered = raw
            .lowercased()
            .trimmingCharacters(in: .whitespacesAndNewlines)

        let punctuation = CharacterSet(charactersIn: "'’`.,:;|/\\-_()[]{}*&^%$#@!~+=<>?")
        let cleaned = lowered.unicodeScalars.map { punctuation.contains($0) ? " " : String(Character($0)) }.joined()
        let compacted = cleaned.replacingOccurrences(of: "\\s+", with: " ", options: .regularExpression)

        let tokens = compacted
            .split(separator: " ")
            .filter { !merchantNoiseTokens.contains(String($0)) }
            .prefix(4)

        return tokens.joined(separator: " ")
    }

    private static let merchantNoiseTokens: Set<String> = [
        "inc", "llc", "co", "corp", "payment", "purchase", "online", "card", "debit", "credit", "store"
    ]
